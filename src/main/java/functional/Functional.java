...
public class Functional {
	...
	private static TripleFunction<List<Integer>, Integer, Integer, List<Integer>> primes = (acc, k, n) ->
			(k > n) ? acc : curry(Functional.primes).apply(acc.stream().anyMatch(i -> k % i == 0) ? acc
					: Stream.concat(acc.stream(), Stream.of(k)).collect(Collectors.toList())).apply(k + 1)
					.apply(n);
	...
	public static IntConsumer fizzyBuzzy = n -> IntStream.range(1, n)
			.forEach(i -> print.accept(check.apply(i)));

	public static IntFunction<List<String>> fizzBuzzList = n -> IntStream.range(1, n)
			.mapToObj(i -> check.apply(i)).collect(Collectors.toList());

	public static Long fact(Long x) {
		return factorial.apply(x);
	}

	public static <P1, P2, P3, R> Function<P1, Function<P2, Function<P3, R>>> curry(
			TripleFunction<P1, P2, P3, R> f) {
