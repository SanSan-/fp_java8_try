...
public class TransactionManagerTest {
    ...
    @Test
    public void runInTx() {
        Person person = TransactionManager.runInTx(
            () -> Person.withInsuranceName("Monty").withAge(27).withName("Colin"), 1000L);
        assertNotNull(person);
        assertEquals("Monty", person.getCar().flatMap(Car::getInsurance).map(Insurance::getName).orElse(""));
